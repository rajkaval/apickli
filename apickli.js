...
const MemCache = require('yl-mem-cache');
...
const CachedResponses = new MemCache('Apickli');
Apickli.prototype.sendRequest = function(method, resource, callback) {
  ...
  if(!CachedResponses.has(cacheKey)){
	request(options,function(error,response){
	  ...
	  CachedResponses.set(cacheKey,response,MemCache.ONE_MINUTE);
      callback(null, response);
	});
  }else {
    self.httpResponse = CachedResponses.get(cacheKey);
	callback(null,CachedResponses.get(cacheKey));
  }
};
